...
public class GazeCamera : MonoBehaviour, IGazeListener
{
    ...
    void Update()
    {
		...
		if (gazeCoords != null && !over) {
			if(gazeCoords.Y > 450 && GameObject.Find("black").GetComponent<GUITexture>().pixelInset.y<-1164f)
			{
				over = true;
			//	GameObject.Find ("black").GetComponent<Moving>().showScore();
				 GameObject.Find ("Lol").GetComponent<GUIText>().text = "YOU PERV";
